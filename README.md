...
Full detail in [`docs/findings.md`](docs/findings.md). Five things are worth
...
5. **An NSG on the injected agent subnet controls Foundry IQ but *not* Web IQ.**
   Denying all Internet egress from `snet-agent` left Grounding with Bing
   working; denying only the Search private endpoint broke the Foundry IQ agent.
   The second result is the control that makes the first believable — the NSG is
   provably enforced, so the Bing call simply never enters the customer network.
   This is the honest answer to "can we add a proxy?": yes for Foundry IQ, no
   for Web IQ. See [`docs/agent-network-flows.md`](docs/agent-network-flows.md).
...
An agent was actually injected into `snet-agent`, and the resulting flows were
measured rather than drawn: see
[`docs/agent-network-flows.md`](docs/agent-network-flows.md). The headline is
that an NSG on that subnet **does** control Foundry IQ retrieval but has **no
effect at all** on Web IQ, because the Bing call is made service-side.
...
# 8. Inject an agent, prove the injection, and run both agents from the subnet.
./scripts/10-agent-flows.sh

# 9. The decisive governance test: can an NSG on the injected subnet block
#    Web IQ while leaving Foundry IQ working? (Removes its own rules.)
./scripts/11-nsg-enforcement.sh

# 10. Ask the services why Work IQ and Fabric IQ cannot run in this tenant.
./scripts/12-workiq-fabriciq-gaps.sh

# 11. Roll everything up into docs/findings.md.
...
scripts/
  ...
  10-agent-flows.sh           C10 — inject an agent, prove it, run both flows
...
src/
  ...
  agent_flow_test.py          runs a Foundry IQ agent and a Web IQ agent
  capability_host_debug.py    follows Azure-AsyncOperation to surface real errors
  probe_search_conn.py        finds the accepted index_connection_id format
docs/
  ...
  agent-network-flows.md      agent -> Foundry IQ and agent -> Web IQ diagrams
  work-iq-requirements.md     what is missing for Work IQ, and how to fix it
  fabric-iq-requirements.md   what is missing for Fabric IQ, and how to fix it
